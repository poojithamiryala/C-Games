...
void initializeBoard(Board * source, int *tobeCopiedArray, int rows, int cols){
	if (source != NULL && tobeCopiedArray != NULL && rows > 0 && cols > 0)
	{
		...
		{
		source->table[i][j] = tobeCopiedArray[j];
		if (tobeCopiedArray[j] == 0 && f==0)
		{
		...
		*tobeCopiedArray++;
		...
		{
			source->table[i / cols][i%cols] = tobeCopiedArray[i];
			if (tobeCopiedArray[i] == 0)
			{
				source->emptyTileRow = i/cols;
				source->emptyTileCol = i%cols;
			}
		}
	}
}
